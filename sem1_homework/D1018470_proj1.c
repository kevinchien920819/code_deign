...
int count,up,down;//count how many times and define down< guess<up
int main(void)
{
   ...
   do {
      ...
      while (isCorrect(guess, answer)!=1) {
         if(isCorrect(guess,answer)==2){
            ...
            down = guess;
            guess = (down + randint(max - min + 1)%200) % up; //let guess number smaller
            if (guess<down)
               guess = guess+down;
            printf("%d\n",guess);  
         }
            ...
            up = guess;
            guess = (guess -randint(max - min + 1)%100) % up;
            printf("%d\n",guess);
         }
...
void correct(void){// print my pig
char pig[][60]={
    "                        /  `.             ",
    "                      ,'     `.           ",
    "       /`.________   (         :          ",
